...
export const fetchBestRoute = async (
    ...
    try {
        ...
        const url = `${SWAP_ROUTE_URL}/best-route?inputAssetAmount=${inputAssetAmount}&inputAssetAddress=${inputAssetAddress}&outputAssetAddress=${outputAssetAddress}${
            options?.maxDepth ? `&maxDepth=${options.maxDepth}` : ""
        }${options?.maxSplits ? `&maxSplits=${options.maxSplits}` : ""}${
            options?.maxSlippage ? `&maxSlippage=${options.maxSlippage}` : ""
        }${
